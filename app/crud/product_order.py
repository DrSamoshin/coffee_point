...
def get_product_order(db: Session, product_order_id: UUID):
    logging.info("call method get_product_order")
    try:
        db_product_order = (
            db.query(ProductOrder).filter(ProductOrder.id == product_order_id).first()
        )
...
def get_product_orders(db: Session):
    logging.info("call method get_product_orders")
    try:
...
def create_product_order(db: Session, product_order: ProductOrderCreate):
    logging.info("call method create_product_order")
    try:
...
def update_product_order(
    db: Session, product_order_id: UUID, updates: ProductOrderUpdate
):
    logging.info("call method update_product_order")
    try:
        db_product_order = (
            db.query(ProductOrder).filter(ProductOrder.id == product_order_id).first()
        )
