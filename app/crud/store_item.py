...
from app.db.models import StoreItem, Supply, Shift
from app.db.db_sessions import db_safe
from app.schemas.store_item import (
    StoreItemCreate,
    StoreItemUpdate,
    StoreItemOut,
    CalculationStoreItemOut,
)


@db_safe
def get_store_item(db: Session, store_item_id: UUID):
    logging.info("call method get_store_item")
    try:
        db_store_item = (
            db.query(StoreItem)
            .filter(StoreItem.id == store_item_id)
            .options(joinedload(StoreItem.supply).joinedload(Supply.supplier))
            .first()
        )
        supplier = None
        if db_store_item.supply and db_store_item.supply.supplier:
            supplier = db_store_item.supply.supplier.name
        store_item = StoreItemOut(
            id=db_store_item.id,
...
def get_store_items(db: Session):
    logging.info("call method get_store_items")
    try:
        db_shift = db.query(Shift).order_by(desc(Shift.start_time)).first()
        store_items = []
        db_store_items = (
            db.query(StoreItem)
            .filter(StoreItem.shift_id == db_shift.id)
            .order_by(desc(StoreItem.date))
            .all()
        )
...
def get_store_items_calculation(db: Session):
    ...
    for store_item in store_items:
        ...
        if not store_items_calculation.get(item_id):
            store_items_calculation[item_id] = {
                "amount": Decimal(0),
                "item_id": item_id,
                "item_name": item_name,
            }
        ...
        if store_item.debit:
            store_items_calculation[item_id]["amount"] = store_items_calculation[
                item_id
            ]["amount"] - Decimal(store_item.amount)
        else:
            store_items_calculation[item_id]["amount"] = store_items_calculation[
                item_id
            ]["amount"] + Decimal(store_item.amount)
    ...
    store_items_calculation = [
        ...
        for key, value in store_items_calculation.items()
    ]
...
def add_store_item(db: Session, store_item: StoreItemCreate):
    logging.info("call method create_store_item")
    try:
        db_shift = db.query(Shift).order_by(desc(Shift.start_time)).first()
        db_store_item = StoreItem(
            item_id=store_item.item_id,
...
def remove_store_item(db: Session, store_item: StoreItemCreate):
    logging.info("call method remove_store_item")
    try:
        db_shift = db.query(Shift).order_by(desc(Shift.start_time)).first()
        db_store_item = StoreItem(
            item_id=store_item.item_id,
...
def update_store_item(db: Session, store_item_id: UUID, updates: StoreItemUpdate):
    logging.info("call method update_store_item")
    print(updates)
    try:
        db_store_item = (
            db.query(StoreItem).filter(StoreItem.id == store_item_id).first()
        )
