...
def get_suppliers(db: Session):
    ...
    try:
        db_suppliers = (
            db.query(Supplier)
            .filter(Supplier.deactivated is False)
...
def get_deactivated_suppliers(db: Session):
    ...
    try:
        db_suppliers = (
            db.query(Supplier)
            .filter(Supplier.deactivated is True)
            .order_by(Supplier.name)
            .all()
        )
    except Exception as error:
...
def delete_supplier(db: Session, supplier_id: UUID):
    ...
    try:
        db_supplier = (
            db.query(Supplier)
            .filter(Supplier.id == supplier_id, Supplier.deactivated is False)
            .first()
        )
...
def activate_supplier(db: Session, supplier_id: UUID):
    ...
    try:
        db_supplier = (
            db.query(Supplier)
            .filter(Supplier.id == supplier_id, Supplier.deactivated is True)
            .first()
        )
        db_supplier.deactivated = False
        db.commit()
