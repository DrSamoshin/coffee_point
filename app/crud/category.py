...
def get_categories(db: Session):
    logging.info("call method get_categories")
    try:
        db_categories = (
            db.query(Category)
            .filter(Category.active is True)
...
def create_category(db: Session, category: CategoryCreate):
    logging.info("call method create_category")
    try:
...
def update_category(db: Session, category_id: UUID, updates: CategoryUpdate):
    logging.info("call method update_category")
    try:
        db_category = (
            db.query(Category)
            .filter(Category.id == category_id, Category.active is True)
            .first()
        )
...
def delete_category(db: Session, category_id: UUID):
    logging.info("call method delete_category")
    try:
