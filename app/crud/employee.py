...
def get_employees(db: Session):
    ...
    try:
        db_employees = (
            db.query(Employee)
            .filter(Employee.deactivated == False)
...
def get_deactivated_employees(db: Session):
    ...
    try:
        db_employees = (
            db.query(Employee)
            .filter(Employee.deactivated == True)
            .order_by(Employee.name)
            .all()
        )
    except Exception as error:
...
def get_available_employees(db: Session):
    ...
    try:
        db_active_shifts = (
            db.query(EmployeeShift).filter(EmployeeShift.active == True).all()
        )
...
def update_employee(db: Session, employee_id: UUID, updates: EmployeeUpdate):
    ...
    try:
        db_employee = (
            db.query(Employee)
            .filter(Employee.id == employee_id, Employee.deactivated == False)
            .first()
        )
...
def activate_employee(db: Session, employee_id: UUID):
    ...
    try:
        db_employee = (
            db.query(Employee)
            .filter(Employee.id == employee_id, Employee.deactivated == True)
            .first()
        )
