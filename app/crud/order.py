...
def create_order_with_products(db: Session, order: OrderCreate):
    logging.info("call method create_order_with_products")
    try:
...
def get_order(db: Session, order_id: UUID):
    logging.info("call method get_order")
    try:
        order = (
            db.query(Order)
...
def get_waiting_shift_orders(db: Session, skip: int = 0, limit: int = 500):
    logging.info("call method get_waiting_shift_orders")
    try:
        db_shift = db.query(Shift).filter(Shift.active is True).first()
        orders = (
            db.query(Order)
...
def update_order_status(db: Session, order_id: UUID, updates: OrderStatusUpdate):
    logging.info("call method update_order_status")
    try:
...
def update_order(db: Session, order_id: UUID, updates: OrderUpdate):
    logging.info("call method update_order")
    try:
        db_order = db.query(Order).filter(Order.id == order_id).first()
            ...
            for product in updates.products:
                if product_order_id := product.product_order_id:
                    product_order = (
                        ...
                        .filter(ProductOrder.id == product_order_id)
                        .first()
                    )
...
def get_products_for_shift_order(db: Session, shift_id: UUID):
    logging.info("call method get_products_for_shift_order")
    try:
        orders = (
            db.query(Order)
            .filter(Order.shift_id == shift_id)
            .options(
                joinedload(Order.product_orders)
                ...
                product_shift_order = {
                    ...
                    "order_date": order.date.strftime(
                        "%Y-%m-%dT%H:%M:%S.{:03d}Z".format(
                            int(order.date.microsecond / 1000)
                        )
                    ),
