...
def get_products(db: Session):
    logging.info("call method get_products")
    try:
        db_products = (
            db.query(Product)
            .filter(Product.active is True)
...
def get_online_shop_products(db: Session):
    logging.info("call method get_online_shop_products")
    try:
        db_products = (
            db.query(Product)
            .filter(Product.active is True, Product.online_shop is True)
            .options(joinedload(Product.category))
            .all()
        )
    except Exception as error:
...
def create_product(db: Session, product: ProductCreate):
    logging.info("call method create_product")
    try:
...
def update_product(db: Session, product_id: UUID, updates: ProductUpdate):
    logging.info("call method update_product")
    try:
...
def delete_product(db: Session, product_id: UUID):
    logging.info("call method delete_product")
    try:
        db_product = db.query(Product).filter(Product.id == product_id).first()
