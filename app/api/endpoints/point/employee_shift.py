...
@router.get(
    "/active-employee-shifts/", response_model=list[EmployeeShiftWithEmployeeOut]
)
async def get_active_employee_shifts(
    db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)
):
...
async def create_employee_shift(
    employee_shifts: EmployeeShiftCreate,
...
async def update_employee_shift_end(
    shift_id: str,
    shift_update: EmployeeShiftUpdate,
    db: Session = Depends(get_point_db),
    user_id: str = Depends(get_user_id_from_token),
):
