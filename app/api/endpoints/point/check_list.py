...
async def get_start_check_list(
    db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)
):
    check_list = crud_check_list.get_check_list(
        db, time_point=CheckListTimePoint.start_shift
    )
    return check_list


@router.get("/end/", response_model=CheckListOut)
async def get_end_check_list(
    db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)
):
    check_list = crud_check_list.get_check_list(
        db, time_point=CheckListTimePoint.end_shift
    )
    return check_list


@router.put("/{check_list_id}/", response_model=CheckListOut)
async def update_start_check_list(
    check_list_id: UUID,
