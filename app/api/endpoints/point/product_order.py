...
async def create_product_order(
    product_order: ProductOrderCreate,
    db: Session = Depends(get_point_db),
    user_id: str = Depends(get_user_id_from_token),
):
...
async def update_product_order(
    product_order_id: UUID,
    product_order_update: ProductOrderUpdate,
    db: Session = Depends(get_point_db),
    user_id: str = Depends(get_user_id_from_token),
):
    db_product_order = crud_product_order.update_product_order(
        db, product_order_id, product_order_update
    )
    return db_product_order
