...
router = APIRouter(prefix='/recipe-items', tags=['recipe_items'])

@router.get("/", response_model=list[RecipeItemOut])
async def get_recipe_items(db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
...
async def get_recipe_item(recipe_item_id: UUID, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
...
async def create_recipe_item(recipe_item: RecipeItemCreate, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
    db_recipe_item = crud_recipe_item.create_recipe_item(db, recipe_item)
    return db_recipe_item

@router.put("/{recipe_item_id}/", response_model=RecipeItemOut)
async def update_recipe_item(recipe_item_id: UUID, recipe_item_update: RecipeItemUpdate, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
