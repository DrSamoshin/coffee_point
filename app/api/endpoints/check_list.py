...
async def get_start_check_list(db: Session = Depends(get_db), user_id: str = Depends(get_user_id_from_token)):
    ...
    check_list_array = check_list.check_list.split(",")
    return CheckListOut(id=check_list.id,
...
async def update_start_check_list(check_list_id: UUID,
    ...
    check_list = crud_check_list.update_check_list(db,
                                                   check_list_id, check_list_update)
    check_list_array = check_list.check_list.split(",")
    return CheckListOut(id=check_list.id,
                        check_list=check_list_array)
