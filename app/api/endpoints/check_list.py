...
async def get_start_check_list(db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
...
async def get_end_check_list(db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
