...
async def upload_image(file: UploadFile = File(...), user_id: str = Depends(get_user_id_from_token)):
    url = await crud_files.upload_image(file)
    return {"url": url}

@router.post("/delete/")
async def delete_image(request: ImageDelete, user_id: str = Depends(get_user_id_from_token)):
