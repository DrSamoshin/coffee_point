...
async def get_supplies(db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
    db_supplies = crud_supply.get_supplies(db)
    return db_supplies

@router.get("/{supply_id}/", response_model=SupplyOut)
async def get_supply(supply_id: UUID, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
    db_supply = crud_supply.get_supply(db, supply_id)
    return db_supply

@router.post("/", response_model=SupplyOut)
async def create_supply(supply: SupplyCreate, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
    db_supply = crud_supply.create_supply(db, supply)
    return db_supply

@router.put("/{supply_id}/", response_model=SupplyOut)
async def update_supply(supply_id: UUID, supply_update: SupplyUpdate, db: Session = Depends(get_point_db), user_id: str = Depends(get_user_id_from_token)):
