...
def read_supplys(db: Session = Depends(get_db), user_id: str = Depends(get_user_id_from_token)):
    return crud_supply.get_supplys(db)

@router.get("/{supply_id}", response_model=SupplyOut)
def read_supply(supply_id: UUID, db: Session = Depends(get_db), user_id: str = Depends(get_user_id_from_token)):
...
def create_supply(supply: SupplyCreate, db: Session = Depends(get_db), user_id: str = Depends(get_user_id_from_token)):
    db_supply = crud_supply.create_supply(db, supply)
    return db_supply

@router.put("/{supply_id}", response_model=SupplyOut)
def update_supply(supply_id: UUID, supply_update: SupplyUpdate, db: Session = Depends(get_db), user_id: str = Depends(get_user_id_from_token)):
    db_supply = crud_supply.get_supply(db, supply_id)
    if not db_supply:
        return response("supply not found", 404)
